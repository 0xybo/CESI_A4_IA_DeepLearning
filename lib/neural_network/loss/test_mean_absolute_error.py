"""
Unit tests for the Mean Absolute Error loss function.
"""
...
def test_mean_absolute_error_compute():
    """
    Test the compute method of the Mean Absolute Error loss function with a simple example.
    """
    ...
    assert np.isclose(loss_value, expected_loss), (
        "Mean Absolute Error loss calculation is incorrect.",
...
def test_mean_absolute_error_derivative():
    """
    Test the derivative method of the Mean Absolute Error loss function with a simple example.
    """
    loss = MeanAbsoluteError()
    y_true = np.array([1.0, 2.0, 3.0])
    y_pred = np.array([1.5, 2.5, 3.5])
    ...
    assert np.allclose(loss_derivative, expected_derivative), (
        "Mean Absolute Error derivative calculation is incorrect.",
